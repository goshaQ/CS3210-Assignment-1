...
void gaussian_blur(unsigned char *src, float *dst, int width, int height, float sigma, int ksize, int n)
{
    int x, y, i, x1, y1;
    ...
    // blur each row
    #pragma omp parallel private (buffer, y, x1) num_threads (n)
    {
    ...
    // blur each column
    #pragma omp parallel private (buffer, x, y1) num_threads (n)
    {
        buffer = (float *) malloc(ksize * sizeof(float));
...
int main(int argc, char ** argv)
{
    ...
    #pragma omp parallel sections
    {
        #pragma omp section
        {
            gaussian_blur(dataB, dstB, width, height, sigma, blur_size, n);
        }
        #pragma omp section
        {
            gaussian_blur(dataR, dstR, width, height, sigma, blur_size, n);
        }
        #pragma omp section
        {
            gaussian_blur(dataG, dstG, width, height, sigma, blur_size, n);
        }
    }
